#include <string.h>

#include "sha224.h"
#include "inttypes.h"

uint8 client_hello_bin[] = {
  
};

uint8 client_hello_bin_hash[] = {
  0xa0, 0x98, 0x64, 0xb4, 0xb0, 0x88, 0x1a, 0xb6,
  0x06, 0x05, 0xbd, 0xa1, 0xee, 0x7d, 0xc1, 0x57,
  0xef, 0xdd, 0x2c, 0x19, 0xf0, 0x88, 0xd0, 0xd3,
  0x77, 0xc1, 0x76, 0x12
};

int main(void)
{
  uint8 h[28];
  sha224_digest(client_hello_bin, sizeof client_hello_bin, h);
  return memcmp(h, client_hello_bin_hash, sizeof h);
}
