#include <string.h>

#include "sha1.h"
#include "inttypes.h"

uint8 client_hello_bin[] = {
  
};

uint8 client_hello_bin_hash[] = {
  0x01, 0x75, 0x53, 0xe4, 0xb8, 0x19, 0x33, 0x91, 0x9e, 0x3e,
  0x3d, 0x5f, 0x0a, 0xdf, 0x5c, 0x73, 0xdd, 0x26, 0x46, 0x05
};

int main(void)
{
  uint8 h[20];
  sha1_digest(client_hello_bin, sizeof client_hello_bin, h);
  return memcmp(h, client_hello_bin_hash, sizeof h);
}
