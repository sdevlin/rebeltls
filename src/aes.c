#include <string.h>

#include "aes.h"

static uint32 rotr(uint32 w, unsigned int k)
{
  return (w >> k) | (w << (32-k));
}

static const byte S[] = {
  
};

static uint32 sub_word(uint32 w)
{
  int i;
  byte *b;
  b = (byte *)&w;
  for (i = 0; i < 4; i += 1) {
    b[i] = S[b[i]];
  }
  return w;
}

static uint32 rot_word(uint32 w)
{
  return rotr(w, 8);
}

static byte Rcon[] = {
  0x00,                         /* unused */
  0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
  0x6c, 0xd8, 0xab, 0x4d        /* only for larger keys */
};

void aes_init(aes_ctx *ctx, byte *key)
{
  int i;
  uint32 *w;
  w = ctx->w;
  memcpy(w, key, 4 * Nk);
  for (i = Nk; i < (Nb * (Nr+1)); i += 1) {
    uint32 tmp = w[i-1];
    if ((i % Nk) == 0) {
      tmp = sub_word(rot_word(tmp)) ^ Rcon[i/Nk];
    }
    w[i] = w[i-Nk] ^ tmp;
  }
}

static void sub_bytes(byte state[4*Nb])
{
  int i;
  for (i = 0; i < 4*Nb; i += 1) {
    state[i] = S[state[i]];
  }
}

static uint32 get_row(byte state[4*Nb], int i)
{
  return ((state[i]) |
          (state[i+4] << 8) |
          (state[i+8] << 16) |
          (state[i+12] << 24));
}

static void set_row(byte state[4*Nb], int i, uint32 w)
{
  int j;
  state[i] = w & 0xff;
  for (j = 1; j < Nb; j += 1) {
    state[i+(j*4)] = (w & (0xff << (j*8))) >> (j*8);
  }
}

static void shift_rows(byte state[4*Nb])
{
  int i;
  uint32 w;
  for (i = 1; i < Nb; i += 1) {
    w = get_row(state, i);
    set_row(state, i, rotr(w, i * 8));
  }
}

static byte M2[] = {
  
};

static byte M3[] = {
  
};

static void mix_columns(byte state[4*Nb])
{
  int i;
  uint32 tmp;
  byte *b;
  for (i = 0; i < Nb; i += 1) {
    tmp = *(uint32 *)(state + (i*4));
    b = (byte *)&tmp;
    state[0+(i*4)] = M2[b[0]] ^ M3[b[1]] ^ b[2] ^ b[3];
    state[1+(i*4)] = b[0] ^ M2[b[1]] ^ M3[b[2]] ^ b[3];
    state[2+(i*4)] = b[0] ^ b[1] ^ M2[b[2]] ^ M3[b[3]];
    state[3+(i*4)] = M3[b[0]] ^ b[1] ^ b[2] ^ M2[b[3]];
  }
}

static void add_round_key(byte state[4*Nb], uint32 w[Nb])
{
  int i;
  uint32 *wp;

  for (i = 0, wp = (uint32 *)state; i < Nb; i += 1) {
    wp[i] ^= w[i];
  }
}

void aes_encrypt(aes_ctx *ctx, byte *p, byte *c)
{
  int i;
  uint32 *w;

  memcpy(c, p, 4*Nb);

  w = ctx->w;
  add_round_key(c, w);

  for (i = 1; i < Nr; i += 1) {
    sub_bytes(c);
    shift_rows(c);
    mix_columns(c);
    add_round_key(c, w + (i*4));
  }

  sub_bytes(c);
  shift_rows(c);
  add_round_key(c, w + (i*4));
}
