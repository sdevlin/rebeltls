...
hmac_ctx *hmac_new(const hash_desc *desc, const u8 *key, uint keylen)
{
  hmac_ctx *ctx;
...
void hmac_update(hmac_ctx *ctx, const u8 *m, uint mlen)
{
  ctx->hash_desc->update(&ctx->tmpctx, m, mlen);
}

void hmac_final(hmac_ctx *ctx, u8 *h)
{
...
void hmac_digest(const hash_desc *desc, const u8 *key, uint keylen,
                 const u8 *m, uint mlen, u8 *h)
{
  hmac_ctx ctx;
  hmac_init(&ctx, desc, key, keylen);
