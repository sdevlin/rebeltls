...
void aes128_init(aes128_ctx *ctx, const byte *key)
{
  ...
  for (i = 4; i < 44; i += 1) {
    uint32 tmp = w[i-1];
    if ((i % 4) == 0) {
      tmp = sub_word(rot_word(tmp)) ^ Rcon[i/4];
    }
    w[i] = w[i-4] ^ tmp;
  }
}
...
static uint32 get_row(byte *state, int i)
{
  return ((state[i]) |
          (state[i+4] << 8) |
          (state[i+8] << 16) |
...
static void set_row(byte *state, int i, uint32 w)
{
  int j;
...
static void shift_rows(byte *state)
{
  ...
  for (i = 1; i < 4; i += 1) {
    w = get_row(state, i);
    set_row(state, i, rotr(w, i * 8));
  }
}

static void add_round_key(byte *state, const uint32 *w)
{
  bytes_xor(state, state, (byte *)w, 16);
}

static uint32 step(uint32 a, uint32 b, uint32 c, uint32 d, uint32 k)
{
  return (T0[(a >> 24) & 0xff] ^
          T1[(b >> 16) & 0xff] ^
          T2[(c >>  8) & 0xff] ^
...
#define ROUND(e, a) do {                                                \
    e[0] = step(a[0], a[1], a[2], a[3], *w++);                          \
    e[1] = step(a[1], a[2], a[3], a[0], *w++);                          \
    e[2] = step(a[2], a[3], a[0], a[1], *w++);                          \
    e[3] = step(a[3], a[0], a[1], a[2], *w++);                          \
...
void aes128_encrypt(const aes128_ctx *ctx, const byte *p, byte *c)
{
  ...
  m[2] ^= *w++;
  m[3] ^= *w++;
  /* bytes_xor((byte *)m, (byte *)m, (byte *)w, sizeof m); */
  /* w += 4; */
  write(STDOUT_FILENO, m, sizeof m);
...
static void inv_shift_rows(byte *state)
{
  ...
  for (i = 1; i < 4; i += 1) {
    w = get_row(state, i);
    set_row(state, i, rotr(w, 32 - (i*8)));
  }
}

static const byte inv_S[] = {
...
static void inv_mix_columns(byte *state)
{
  ...
  for (i = 0; i < 4; i += 1) {
    tmp = *(uint32 *)(state + (i*4));
    b = (byte *)&tmp;
    state[0+(i*4)] = Me[b[0]] ^ Mb[b[1]] ^ Md[b[2]] ^ M9[b[3]];
    state[1+(i*4)] = M9[b[0]] ^ Me[b[1]] ^ Mb[b[2]] ^ Md[b[3]];
    state[2+(i*4)] = Md[b[0]] ^ M9[b[1]] ^ Me[b[2]] ^ Mb[b[3]];
    state[3+(i*4)] = Mb[b[0]] ^ Md[b[1]] ^ M9[b[2]] ^ Me[b[3]];
  }
}
...
void aes128_decrypt(const aes128_ctx *ctx, const byte *c, byte *p)
{
  ...
  w = ctx->w;
  add_round_key(p, w + 40);

  for (i = 9; i > 0; i -= 1) {
    inv_shift_rows(p);
    inv_sub_bytes(p);
    add_round_key(p, w + (i*4));
    inv_mix_columns(p);
  }
