...
#define DEC_ROUND(e, a) do {                            \
    e[0] = dec_roundstep(a[0], a[3], a[2], a[1], *w++); \
    e[1] = dec_roundstep(a[1], a[0], a[3], a[2], *w++); \
    e[2] = dec_roundstep(a[2], a[1], a[0], a[3], *w++); \
    e[3] = dec_roundstep(a[3], a[2], a[1], a[0], *w++); \
  } while (0)

static uint32 dec_finalroundstep(uint32 a, uint32 b, uint32 c, uint32 d,
                                 uint32 k)
{
  return ((dec_T2[(a >> 24) & 0xff] & 0xff000000) ^
          (dec_T3[(b >> 16) & 0xff] & 0x00ff0000) ^
          (dec_T0[(c >>  8) & 0xff] & 0x0000ff00) ^
          (dec_T1[(d      ) & 0xff] & 0x000000ff) ^
          k);
}
...
void aes128_decrypt(const aes128_ctx *ctx, const byte *c, byte *p)
{
  ...
  /* 10 */
  m[0] = dec_finalroundstep(n[0], n[3], n[2], n[1], *w++);
  m[1] = dec_finalroundstep(n[1], n[0], n[3], n[2], *w++);
  m[2] = dec_finalroundstep(n[2], n[1], n[0], n[3], *w++);
  m[3] = dec_finalroundstep(n[3], n[2], n[1], n[0], *w++);
